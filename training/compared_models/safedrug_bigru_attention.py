...
class SafeDrugModel(nn.Module):
    def __init__(
        ...
        # bipartite transform -> same as before
        ...
        with torch.no_grad():
            mpnn_emb = mpnn(self.MPNN_molecule_Set)  # (num_molecules, emb_dim)
            mpnn_emb = torch.mm(average_projection.to(device=device), mpnn_emb.to(device=device))

        self.register_buffer("MPNN_emb", mpnn_emb)  # shape (num_molecules, emb_dim)

        # Project the concatenated (query + context) to vocab space:
        # query (emb_dim) + context (emb_dim) -> 2 * emb_dim input
    ...
    def forward(self, input):
        ...
        # ---------- Attention over precomputed MPNN embeddings ----------
        # MPNN_emb: (num_molecules, emb_dim)
        # scores: (1, num_molecules)
        scaling = math.sqrt(self.MPNN_emb.size(1))
        scores = torch.matmul(query, self.MPNN_emb.t()) / scaling
        attn_weights = torch.softmax(scores, dim=-1)  # (1, num_molecules)

        # context: weighted sum of molecule embeddings -> (1, emb_dim)
        context = torch.matmul(attn_weights, self.MPNN_emb)  # (1, emb_dim)
